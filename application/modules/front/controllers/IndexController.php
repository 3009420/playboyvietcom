...
// 			$this->view->namefile = $promotedjson->namefile;
        ...
        $this->view->adsindex1 = $appHotgirl->loadAds(1, 1);
        $this->view->adsindex2 = $appHotgirl->loadAds(1, 2);
        $this->view->adsindex3 = $appHotgirl->loadAds(1, 3);
...
// 			echo "</pre>";
        ...
        foreach ($listcontentdetailfull as $cfg) {
            if ((strpos($cfg['image_thumbnail'], $http) === false) and ( $cfg['image_thumbnail'] != "")) {
                $src = WEB_PATH . '/public/uploads/news/' . $cfg['image_thumbnail'];
            }
            if ((strpos($cfg['image_thumbnail'], $http) === 0 ) and ( $cfg['image_thumbnail'] != "")) {
                $src = $cfg['image_thumbnail'];
            }
            //var_dump($src);die;
        ...
        $title = '';
        echo $objUtil->renderData($title, $ajaxData, $paging);
        die();
    }
    ...
    public function detailAction() {
        ...
        $this->view->adsdetail1 = $appHotgirl->loadAds(2, 1);
        $this->view->adsdetail2 = $appHotgirl->loadAds(2, 2);
        $this->view->adsdetail3 = $appHotgirl->loadAds(2, 3);
        $this->view->adsdetail4 = $appHotgirl->loadAds(2, 4);
        $this->view->adsdetail5 = $appHotgirl->loadAds(2, 5);
    }
